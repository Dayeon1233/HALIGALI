...
BEGIN_MESSAGE_MAP(CArrow, CDialogEx)
	...
	ON_WM_TIMER()
	//	ON_COMMAND(ID_HIGH, &CArrow::OnHigh)
	//	ON_UPDATE_COMMAND_UI(ID_HIGH, &CArrow::OnUpdateHigh)
	//	ON_COMMAND(ID_LOW, &CArrow::OnLow)
	//	ON_UPDATE_COMMAND_UI(ID_LOW, &CArrow::OnUpdateLow)
	//	ON_COMMAND(ID_MIDDLE, &CArrow::OnMiddle)
	//	ON_UPDATE_COMMAND_UI(ID_MIDDLE, &CArrow::OnUpdateMiddle)
END_MESSAGE_MAP()
...
BOOL CArrow::OnInitDialog()
{
	...
	if (((CHalliGalli_dlgDlg *)GetParent())->m_level == 1) {//중 난이도일 경우
		for (int i = 0; i < m_arrowcnt; i++) {
			Arrow_piece* ap = new Arrow_piece(rand() % 2);//0,1
			m_arrow_arr.Add(ap);//화살표들을 배열에 추가
		}
	}
	else if (((CHalliGalli_dlgDlg *)GetParent())->m_level == 0) { //상 난이도일 경우
		for (int i = 0; i < m_arrowcnt; i++) {
			Arrow_piece* ap = new Arrow_piece(rand() % 2);//0,1
			m_arrow_arr.Add(ap);//화살표들을 배열에 추가
		}
